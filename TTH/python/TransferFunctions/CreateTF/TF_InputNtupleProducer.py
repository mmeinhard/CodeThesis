...
#This code is adapted from MEAnalysis/python/JetTransfer and produces the Ntuples necessary 
#for calculating the transfer functions. A nanoAOD tree is needed as an input. 
#Only quarks which are matched to a jet are saved.

#Match two lists, remove objects which are matched to two objects (to make sure that the same jet 
    #is not matched to more than one quark)
...
for i in range(tree.GetEntries()):
    ...
    for quark_idx in matches_by_quark.keys():
        ...
        if quark.num_matches > 0:
            ...
            quark.match_btagDeepCSV = best_match.btagDeepCSV
            quark.match_dr = dr

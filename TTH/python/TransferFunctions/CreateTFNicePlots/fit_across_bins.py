...
def Make_AcrossBinFit( TFobj, fit_dicts, config ):
    ...
    for (i_abfunc, abfunc) in enumerate( TFobj.AcrossBinFuncs ):
        ...
        ROOT.gStyle.SetOptStat(00000000)
        ROOT.gStyle.SetStatX(44)
        ROOT.gStyle.SetStatY(8.3)

        #pt = c1.GetListOfPrimitives().FindObject("title")
        #pt.SetX1NDC(0.4)
        #pt.SetX2NDC(0.7)
        #pt.SetY1NDC(0.15)
        #pt.SetY2NDC(0.25)
        ...
        gr.GetXaxis().SetTitle("Quark p_{T} [GeV]")
        gr.GetYaxis().SetTitle( "#alpha_{1}")
...
#
        ...
        c1.Print('{0}/TFs/{1}.png'.format( config['outputdir'], filename ) ,".png")
        c1.Print('{0}/TFs/{1}.pdf'.format( config['outputdir'], filename ) ,".pdf")
        #img = ROOT.TImage.Create()
        #img.FromPad(c1)
        #img.WriteImage('{0}/TFs/{1}.png'.format( config['outputdir'], filename ) )
        #img.WriteImage('{0}/TFs/{1}.pdf'.format( config['outputdir'], filename ) )

...
varnames = {
    ...
    "jetsByPt_4_pt": r"fifth jet $p_T$ [GeV]",
    "jetsByPt_5_pt": r"sixth jet $p_T$ [GeV]",
    ...
    "jetsByPt_4_btagCSV": r"fifth jet CSV",
    "jetsByPt_5_btagCSV": r"sixth jet CSV",
    ...
    "jetsByPt_4_eta": r"fifth jet $\eta$",
    "jetsByPt_5_eta": r"sixth jet $\eta$",

    "jetsByPt_0_phi": r"leading jet $\phi$",
    "jetsByPt_1_phi": r"subleading jet $\phi$",
    "jetsByPt_2_phi": r"third jet $\phi$",
    "jetsByPt_3_phi": r"fourth jet $\phi$",
    "jetsByPt_4_phi": r"fifth jet $\phi$",
    "jetsByPt_5_phi": r"sixth jet $\phi$",

    "jetsByPt_0_mass": r"leading jet mass",
    "jetsByPt_1_mass": r"subleading jet mass",
    "jetsByPt_2_mass": r"third jet mass",
    "jetsByPt_3_mass": r"fourth jet mass",
    "jetsByPt_4_mass": r"fifth jet mass",
    "jetsByPt_5_mass": r"sixth jet mass",
    ...
    "leps_0_phi": r"leading lepton $\phi$",
    "leps_0_mass": r"leading lepton mass",
    ...
    "btag_LR_4b_2b_btagCSV_logit_fine" : r"b tagging likelihood ratio, $\mathcal{BLR}$",
    ...
    "mem_SL_0w2h2t_p_fine": r"MEM discriminant, $P_{\mathrm{s/b}}$",
    "mem_SL_1w2h2t_p_fine": r"MEM discriminant, $P_{\mathrm{s/b}}$",
    "mem_SL_2w2h2t_p_fine": r"MEM discriminant, $P_{\mathrm{s/b}}$",
    "mem_DL_0w2h2t_p_fine": r"MEM discriminant, $P_{\mathrm{s/b}}$",
    ...
    "avgdetabtag": r"average $\eta$ between b-tagged jets",
    "avgdeta": r"average $\eta$ between jets",
    ...
    "minpt":r"$p_{T}$ of sum of two closest jets [GeV]",
    "avgpt":r"Avg jet $p_{T}$ [GeV]",
    "avgmassbtag":r"Avg mass of b-tagged jets [GeV]",
    "massjetlep":r"Mass of lep and closest jet [GeV]",
    "avgbtag":r"Avg jet btag",
    "avgbtagbtag":r"Avg btag of b-tagged jets",
    "minmass":r"Mass of two closest b-tagged jets",
...
def mc_stack(
    ...
    for i in range(1,htot.GetNbinsX()+1):
        err1p.append(htot.GetBinError(i)/2)
    err1 = np.array(err1p)
    err2 = err1
...
def draw_data_mc(tf, hname, processes, signal_processes, **kwargs):
    ...
    histograms_highmemMC = getHistograms(tf, [('all', 'all')], hname.replace("sl_jge6_tge4__", "sl_jge6_tge4_highmem__"), pattern=pattern, rename_func=rename_func, postprocess_hist=postprocess_hist)

    histograms_highmemData = getHistograms(tf, [('data', 'data')], hname.replace("sl_jge6_tge4__", "sl_jge6_tge4_highmem__"), pattern=pattern, rename_func=rename_func, postprocess_hist=postprocess_hist)
    ...
    histograms_highmemMC["all"].Scale(50.0)
    histograms_highmemMC["all"].title = "MC, MEM > 0.85" + " x50"
    histograms_highmemMC["all"].linewidth=2
    histograms_highmemMC["all"].color = "red"
    histograms_highmemMC["all"].fillstyle = None


    hist(histograms_highmemMC["all"])

    histograms_highmemData["data"].Scale(50.0)
    histograms_highmemData["data"].title = "Data, MEM > 0.85" + " x50"
    histograms_highmemData["data"].linewidth=2
    histograms_highmemData["data"].color = "black"
    histograms_highmemData["data"].fillstyle = None


    hist(histograms_highmemData["data"])
    ...
    if do_legend:
        ...
        patches += [mlines.Line2D([], [], color=histograms_highmemMC["all"].color[0], label=histograms_highmemMC["all"].title, linewidth=2)]
        patches += [mlines.Line2D([], [], color=histograms_highmemData["data"].color[0], label=histograms_highmemData["data"].title, linewidth=2)]

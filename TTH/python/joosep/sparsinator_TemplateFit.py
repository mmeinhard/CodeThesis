    ...
    if analysis.config.getboolean("sparsinator", "add_systematics"):
        ...
        for sdir in ["Up", "Down"]:
            for syst in systematics_btag:
                ...
                systematic_weights += [
                    (bweight, lambda ev, bweight=bweight, syst_pairs=syst_pairs: (np.sign(ev.weights.at(syst_pairs["gen"])) * ev.weights.at(syst_pairs["CMS_pu"]) *
                ...
                systematic_weights += [
                    (bweight_boosted, lambda ev, bweight=bweight, bweight_boosted=bweight_boosted, syst_pairs=syst_pairs: (np.sign(ev.weights.at(syst_pairs["gen"])) * ev.weights.at(syst_pairs["CMS_pu"]) *
        ...
        for lep_syst in [
            ...
            systematic_weights += [
                (lep_syst, lambda ev, syst_pairs=syst_pairs, lep_syst=lep_syst: (
                    np.sign(ev.weights.at(syst_pairs["gen"])) * ev.weights.at(syst_pairs["CMS_pu"]) *
        ...
        while events.reader.Next():
            ...
            for iSyst, syst in enumerate(systematics_event):
                ...
                if schema == "mc" or schema == "mc_syst":
                    ...
                    #if syst == "nominal":
                    #    print("{8} - {0}: pu={1} gen={2}(actual val = {3}) btag={4} btagNorm={5} L1Prefiring={6} leptonweight={7}".format(
                    #        syst,
                    ...
                    event.weight_nominal *= (event.weights.at(syst_pairs["CMS_pu"]) * np.sign(event.weights.at(syst_pairs["gen"])) * event.lepton_weight)
            
                    event.weight_btag_nominal = 1
                ...
                #dilepton specific cuts
                    ...
                    if math.copysign(1, event.leptons.at(0).pdgId * event.leptons.at(1).pdgId) == 1:
                        continue
                    #same flavour
                    if event.jets.at(0).btagDeepCSV < 0.4941:
                        continue
                    if abs(event.leptons.at(0).pdgId) == abs(event.leptons.at(1).pdgId):
                        if event.met_pt <= 30 or abs(mll - 91) < 15:
                            continue
...
if __name__ == "__main__":
    ...
    if os.environ.has_key("FILE_NAMES"):
        ...
        skip_events = 0
        max_events = -1
        analysis = analysisFromConfig(os.environ["CMSSW_BASE"] + "/src/TTH/MEAnalysis/data/default_TemplateFit.cfg")
        ...
        analysis = analysisFromConfig(os.environ["CMSSW_BASE"] + "/src/TTH/MEAnalysis/data/default_TemplateFit.cfg")
        file_names = analysis.get_sample(sample).file_names
        #file_names = ["root://t3dcachedb.psi.ch//pnfs/psi.ch/cms/trivcat/store/user/mameinha/tth/may15_v1/SingleMuon/may15_v1/200515_162756/0000/tree_546.root"]
        ...
        #file_names = ["/work/mameinha/testcrab/CMSSW_10_2_15_patch2/src/TTH/MEAnalysis/python/Loop_TTTo2LNu_test/tree.root"]
